  ...
  async function loadAll() {
    ...
    for (var i = 0; i < MAIN_CATEGORIES.length; i++) {
      ...
      items.forEach(function (item) {
        ...
        if (mainCat.key === "hizmetler") {
          var allowedHizmetler = ["Kuaför", "Berber", "Veteriner", "Akaryakıt"];
          if (allowedHizmetler.indexOf(item.name) === -1) {
            return;
          }
        }
        if (mainCat.key === "kultur") {
          var allowedKultur = ["Müzeler", "Tarihi Camiler"];
          if (allowedKultur.indexOf(item.name) === -1) {
            return;
          }
        }
        allChips.push({
