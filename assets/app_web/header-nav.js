  ...
  function createMobileBottomNav(options = {}) {
      ...
      return (
        {
          ...
          DE: "In der Nähe",
          ZH: "附近",
        }[language] || "Yakındakiler"
      );
    };
    const labels = getNavLabels();
    const nearbyLabel = getNearbyLabel();
    const authLabels =
    ...
    wrapper.innerHTML = `
      <nav class="mobile-bottom-nav-actions" aria-label="${labels.nav}">
        ...
        <button class="mobile-bottom-nav-btn" data-mobile-nav="nearby" type="button" aria-label="${nearbyLabel}" title="${nearbyLabel}">
    ...
    document.addEventListener("aramabul:languagechange", () => {
      ...
      const nearbyBtn = wrapper.querySelector('[data-mobile-nav="nearby"]');
      ...
      if (nearbyBtn) {
        const nextNearbyLabel = getNearbyLabel();
        nearbyBtn.setAttribute("aria-label", nextNearbyLabel);
        nearbyBtn.setAttribute("title", nextNearbyLabel);
      }
      if (profileBtn) {
